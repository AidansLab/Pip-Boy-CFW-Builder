...
let submenuCamera = () =>
{
    ...
    drawFooter = () =>
    {
        ...
        if(VUSB_PRESENT.read())
        {
            ...
            // Send first Gary! immediately
            console.log("Gary!");
            
            ConnectionTimeout = setInterval(function() {
                handshakeAttempts++;
                ...
                if(camConnected) {
                    ...
                    ConnectionTimeout = null;
                    bF.setFontAlign(1, -1);
                    bF.drawString("Ready", bF.getWidth()-10, 0).flip();
                } else if(handshakeAttempts >= maxAttempts) {
                    clearInterval(ConnectionTimeout);
                    ConnectionTimeout = null;
                    bF.setFontAlign(1, -1);
                    bF.drawString("Off", bF.getWidth()-10, 0).flip();
                } else {
                    console.log("Gary!");
                }
            }, 200);
