...
let submenuCamera = () =>
{
    ...
    // 2. Force garbage collection to ensure we have a huge contiguous block
    process.memory(true); 

    // 3. Create a NEW 4bpp buffer (400x210 x 4 bits = ~42KB RAM)
    // We use msb:true to match the format generated by the converter script
    bC = Graphics.createArrayBuffer(400, 210, 4, { msb: true });

    // 4. Restore the flip function (required to send data to the screen)
    bC.flip = function(y) { 
    ...
    {
        // 1. Open the file
        const f = E.openFile("IMGS/image_1.img", "r");
        ...
        // 5. Draw Loop (Row by Row)
        // We only draw if the line is within the screen vertical bounds
        for (let y = 0; y < imgH; y++) {
            let chunk = f.read(bytesPerRow);
            if (!chunk) break;

            // Only draw if inside vertical bounds (simple clipping)
    ...
    function handleLeftKnob(dir)
    {
        if (dir == 0)
        {
            console.log("Left knob pressed");
        } else {
            console.log("Left knob turned: " + dir);
        }
    }

    function handleRightKnob(dir)
    {
        console.log("Right knob turned: " + dir);
    }
    ...
    {
        console.log("Tune Up pressed");
    }

    function handleTuneDown()
    {
        console.log("Tune Down pressed");
    }
    ...
    TuneButtonInterval = setInterval(function ()
    {
    ...
    TuneUpInterval = setInterval(function ()
    {
        const currentState = BTN_TUNEUP.read();
        if (currentState && !lastTuneUpState)
        {
            handleTuneUp();
        }
        lastTuneUpState = currentState;
    }, 100);

    TuneDownInterval = setInterval(function ()
    {
        const currentState = BTN_TUNEDOWN.read();
        if (currentState && !lastTuneDownState)
        {
            handleTuneDown();
        }
        lastTuneDownState = currentState;
    }, 100);
